# 动手前必读
## 1、使用公有云服务器
阿里云不支持虚拟IP，**跳过keepalived的章节，不要用虚拟ip**（云环境一般都不支持自己定义虚拟ip）就直接把虚拟ip设置为第一个master的ip就可以了（global-config.properties文件中配置【下文第五节 - 准备配置文件】）。  
...
```bash
$ cd ~
$ git clone https://github.com/Farmerddd/kubernetes-ha-binary.git
